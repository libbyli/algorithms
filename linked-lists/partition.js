...
const partition = (x, node) => {
  const lesserList = LinkedList();
  const greaterList = LinkedList();

  while (node) {
    if (node.value < x) {
      lesserList.addToTail(node.value);
    } else {
      greaterList.addToTail(node.value);
    }
    node = node.next;
  }

  lesserList.tail.next = greaterList.head;
  return lesserList;
};
